...
def read_data(tuned, task):
    ...
    for model in models:
        model_df = pd.read_csv(
            f"{RESULT_PATH}/{task}/{model}_{tuned}_saliency.csv", sep=","
        )
        ...
        if col not in ["sid", "fixation"]:
            for i in range(len(df)):
                if len(df[col][i]) != len(df["fixation"][i]):
                    raise ValueError(
                        f"Length of {col} and fixation is not the same for {i}"
                    )
...
def subject_analysis(task, model_name):
    ...
    model_df = pd.read_csv(
        f"{RESULT_PATH}/{task}/{model_name}_finetuned_saliency.csv", sep=","
    )
...
def draw_subject_barplot(task_names=["sst", "wiki"], model_name="gpt2"):
    ...
    for j, task in enumerate(task_names):
        mean_df = pd.read_csv(
            f"{RESULT_PATH}/{task}/{model_name}_subj_corr.csv", sep=","
        )
        mean = mean_df.mean()
        std_error = mean_df.std() / np.sqrt(len(mean_df))
        print(task)
        # print the value in mean
        for key, value in mean.items():
            # round value to two decimal
            value = round(value, 2)
            print("& " + str(value), end=" ")
        print("avg: ", str(mean.mean()))

        if task == "sst":
            mean = mean.sort_index(key=lambda x: [subj_sst_acc[subj] for subj in x])
            std_error = std_error.sort_index(
                key=lambda x: [subj_sst_acc[subj] for subj in x]
            )
        else:
            mean = mean.sort_index(key=lambda x: [subj_wiki_acc[subj] for subj in x])
            std_error = std_error.sort_index(
                key=lambda x: [subj_wiki_acc[subj] for subj in x]
            )
        ...
        axs[j].set_ylim(0.1, 0.5)
        if j == 0:
            axs[j].set_ylabel("Spearman Correlation", fontsize=12)

    plt.tight_layout()
    plt.savefig(f"{RESULT_PATH}/{model_name}_barplot.png", dpi=300)
