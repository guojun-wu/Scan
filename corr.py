...
def avergae_saliency(path):
    df = pd.read_csv(path, sep=',')
    df_avg = pd.DataFrame(columns=['sn', 'x_explanation', 'l1_explanation'])
    sentences = df['sn'].unique()
    for sentence in sentences:
        df_sentence = df[df['sn'] == sentence]
        df_tmp = df_sentence['x_explanation'].apply(ast.literal_eval)
        avg_x = np.mean(df_tmp.values.tolist(), axis=0)
        df_tmp = df_sentence['l1_explanation'].apply(ast.literal_eval)
        avg_l1 = np.mean(df_tmp.values.tolist(), axis=0)
        df_avg = pd.concat([df_avg, pd.DataFrame({'sn': sentence, 'x_explanation': [avg_x.tolist()], 'l1_explanation': [avg_l1.tolist()]})], ignore_index=True)
    return df_avg
...
def main():
    ...
    x_explanation = df['x_explanation']
    l1_explanation = df['l1_explanation']
